...
/**
 * Renders an atmosphere around the globe and a sky dome at low altitude. Uses
 * atmospheric scattering as color source.
 * <p>
 * Issue : Ellipsoidal globe doesnt match the spherical atmosphere everywhere.
 * <p>
 * TODO: Find a way to get a blue sky at ground level TODO: Increase dome
 * geometry resolution and implement partial sphere
 *
...
public class AtmosphereLayer extends AbstractLayer
{
  ...
  public void doRender(DrawContext dc)
  {
    ...
    try
    {
      ...
      Position camPos = dc.getGlobe().computePositionFromPoint(
          view.getEyePoint());
      ...
      // FP
      double tangentalDistance = view.getHorizonDistance();

      // Dome radius
      double domeRadius = tangentalDistance;

      // horizon latitude degrees
      double horizonLat = (-Math.PI / 2 + Math.acos(tangentalDistance
      ...
      if (camAlt >= thickness)
      {
        double tangentalDistanceZenith = Math.sqrt(distToCenterOfPlanet
            * distToCenterOfPlanet - (worldRadius + thickness)
            * (worldRadius + thickness));
        zenithLat = (-Math.PI / 2 + Math.acos(tangentalDistanceZenith
            / distToCenterOfPlanet))
            * 180 / Math.PI;
      }
      ...
      if (this.update || this.glListId == -1
          || Math.abs(this.lastRebuildHorizon - tangentalDistance) > 100)
      {
        if (this.glListId != -1)
          gl.glDeleteLists(this.glListId, 1);

        this.makeSkyDome(dc, (float) (domeRadius), horizonLat, zenithLat,
            SLICES, STACKS);
      ...
      gl.glPushAttrib(GL2.GL_COLOR_BUFFER_BIT | GL2.GL_DEPTH_BUFFER_BIT
          | GL2.GL_TRANSFORM_BIT | GL2.GL_POLYGON_BIT | GL2.GL_TEXTURE_BIT
      ...
      Matrix projection = Matrix.fromPerspective(view.getFieldOfView(), view
          .getViewport().getWidth(), view.getViewport().getHeight(), 10e3,
   ...
   * @param radius
   *          the sky dome radius in meters.
   * @param startLat
   *          the horizon latitude in decimal degrees.
   * @param endLat
   *          the zenith latitude in decimal degrees.
   * @param slices
   *          the number of longitude divisions used for the dome geometry.
   * @param stacks
   *          the number of latitude divisions used for the dome geometry.
   */
  protected void makeSkyDome(DrawContext dc, float radius, double startLat,
      double endLat, int slices, int stacks)
  {
   ...
   */
  protected void drawSkyGradient(DrawContext dc, float radius, double startLat,
      double endLat, int slices, int stacks)
  {
    // Init atmospheric scattering computer
    if (this.asc == null)
      this.asc = new AtmosphericScatteringComputer(dc.getGlobe().getRadius(),
          this.thickness);
    ...
    for (int slice = 0; slice <= slices; slice++)
    {
      ...
      color = this.asc.getAtmosphereColor(v2.transformBy4(skyTransform),
          eyePoint, this.sunDirection);
    ...
    for (int stack = 1; stack < stacks - 1; stack++)
    {
      ...
      for (int slice = 0; slice <= slices; slice++)
      {
        ...
        color = this.asc.getAtmosphereColor(v.transformBy4(skyTransform),
            eyePoint, this.sunDirection);
        gl.glColor4f(color.getRed() / 255f, color.getGreen() / 255f,
            color.getBlue() / 255f, color.getAlpha() / 255f);
        gl.glVertex3d(v.getX(), v.getY(), v.getZ());
  ...
  protected Matrix computeSkyTransform(DrawContext dc)
  {
    ...
    // transform = transform.multiply(dc.getGlobe().
    // .computeTransformToPosition(dc.getView().getEyePosition()));
    // FP
    transform = transform
        .multiply(dc.getGlobe().computeModelCoordinateOriginTransform(
            (dc.getView().getEyePosition())));
  ...
  /**
   * Converts position in spherical coordinates (lat/lon/altitude) to cartesian
   * (XYZ) coordinates.
   *
   * @param latitude
   ...
   */
  protected static Vec4 SphericalToCartesian(double latitude, double longitude,
      double radius)
  {
    ...
    return new Vec4(radCosLat * Math.sin(longitude), radius
        * Math.sin(latitude), radCosLat * Math.cos(longitude));
  }
