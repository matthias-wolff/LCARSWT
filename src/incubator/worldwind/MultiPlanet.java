...
public class MultiPlanet extends JFrame
{
  ...
  private static class WWPanel extends JPanel
  {
    ...
    {
      // To share resources among World Windows, pass the first World Window to
      // the constructor of the other
      // World Windows.
  ...
  public MultiPlanet()
  {
    try
    {
      // Create the application frame and the tabbed pane and add the pane to
      // the frame.
      ...
      // );
      load(mars_layers, "USGS",
          "http://www.mapaplanet.org/explorer-bin/imageMaker.cgi?map=Mars",
          "Mars");
      load(mars_layers, "USGS2",
          "http://mars-wms.wr.usgs.gov/cgi-bin/mapserv?map=/var/www/html/mapfiles/mars/mars_simp_cyl.map",
          "Mars");
      load(mars_layers, "USGS3",
          "http://66.85.141.154/cgi-bin/mapserv?map=/var/www/html/mapfiles/mars/mars_simp_cyl_span540.map",
          "Mars");

      // Layers for Venus
      ArrayList<Layer> venus_layers = new ArrayList<Layer>();
      load(venus_layers, "USGS",
          "http://www.mapaplanet.org/explorer-bin/imageMaker.cgi?map=Venus",
          "Venus");

      // Layers for Callisto
      ArrayList<Layer> callisto_layers = new ArrayList<Layer>();
      load(callisto_layers, "USGS",
          "http://www.mapaplanet.org/explorer-bin/imageMaker.cgi?map=Callisto",
          "Callisto");

      // Layers for Europa
      ArrayList<Layer> europa_layers = new ArrayList<Layer>();
      load(europa_layers, "USGS",
          "http://www.mapaplanet.org/explorer-bin/imageMaker.cgi?map=Europa",
          "Europa");

      // Layers for Ganymede
      ArrayList<Layer> ganymede_layers = new ArrayList<Layer>();
      load(ganymede_layers, "USGS",
          "http://www.mapaplanet.org/explorer-bin/imageMaker.cgi?map=Ganymede",
          "Ganymede");

      // Layers for Io
      ArrayList<Layer> io_layers = new ArrayList<Layer>();
      load(io_layers, "USGS",
          "http://www.mapaplanet.org/explorer-bin/imageMaker.cgi?map=Io", "Io");

      // Add Layer Panels
      ArrayList<Layer> all_earth_layers = setLayerPanels(common_layers,
          earth_layers, earth_panel);
      ArrayList<Layer> all_moon_layers = setLayerPanels(common_layers,
          moon_layers, moon_panel);
      ArrayList<Layer> all_mars_layers = setLayerPanels(common_layers,
          mars_layers, mars_panel);
      ArrayList<Layer> all_venus_layers = setLayerPanels(common_layers,
          venus_layers, venus_panel);
      ArrayList<Layer> all_callisto_layers = setLayerPanels(common_layers,
          callisto_layers, callisto_panel);
      ArrayList<Layer> all_europa_layers = setLayerPanels(common_layers,
          europa_layers, europa_panel);
      ArrayList<Layer> all_ganymede_layers = setLayerPanels(common_layers,
          ganymede_layers, ganymede_panel);
      ArrayList<Layer> all_io_layers = setLayerPanels(common_layers, io_layers,
          io_panel);
      ...
      setModel(new Earth()/* Ganymede() */, all_ganymede_layers,
          ganymede_panel);
      ...
      this.pack();
      WWUtil.alignComponent(null, this, AVKey.CENTER); // Center the application
                                                       // on the screen.
  ...
  private ArrayList<Layer> setLayerPanels(ArrayList<Layer> common_layers,
      ArrayList<Layer> layers, WWPanel planet_panel)
  {
    ...
    // Mark the layer as hidden to prevent it being included in the layer tree's
    // model. Including the layer in
    // the tree would enable the user to hide the layer tree display with no way
    // of bringing it back.
  ...
  private void setModel(Globe globe, ArrayList<Layer> layers,
      WWPanel planet_panel)
  {
  ...
  private void setViewControlsLayer(WWPanel planet_panel)
  {
    ...
    planet_panel.wwd.addSelectListener(
        new ViewControlsSelectListener(planet_panel.wwd, view_controls));

    // add also a scale bar
    planet_panel.wwd.getModel().getLayers().add(new ScalebarLayer());
  }

  protected void load(ArrayList<Layer> layers, String source, String url,
      String planet)
  {
    ...
    if (namedLayerCaps != null)
      for (WMSLayerCapabilities lc : namedLayerCaps)
      {
        ...
        planet_layers
            .add(new PlanetLayer(url, planet, title, name, description));
      }
